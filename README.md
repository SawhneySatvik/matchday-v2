...
- `lib/*` core logic (`parseGeminiJSON`, utilities, Zustand cache/session behaviors)
- `data/venues.ts` matching logic
- `app/api/*/route.ts` request validation + success-path contracts with mocked Gemini clients

Coverage reports are generated in `coverage/`.

Baseline threshold policy (enforced in Vitest):
